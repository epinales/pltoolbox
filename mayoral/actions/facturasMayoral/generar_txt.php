...
error_log("Real Header encoding UTF8: " . mb_detect_encoding(utf8_encode($headers[0])) . ", Length: " . strlen(utf8_encode($headers[0])) . " string: " . utf8_encode($headers[0]));
error_log("Real Header encoding: " . mb_detect_encoding($headers[0]) . ", Length: " . strlen($headers[0]) . " string: " . $headers[0]);

for ($i=0; $i < $num_headers; $i++) {
  if (!($headers[$i] == $documented_headers[$i])) {
    $system_callback['code'] = 500;
    $system_callback['message'] = "Los encabezados no son correctos. Se esperaba $documented_headers[$i]; y se encontró: " . utf8_encode($headers[0]);
