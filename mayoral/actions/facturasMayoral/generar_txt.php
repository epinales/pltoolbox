...
if (!$test_extension) {
  ...
  $system_callback['message'] = htmlspecialchars("Los archivos con extensión $extension no están permitidos. Favor de subir un archivo CSV.");
...
for ($i=0; $i < $num_headers; $i++) {
  if (!($headers[$i] == $documented_headers[$i])) {
    $system_callback['code'] = 500;
    $system_callback['message'] = htmlspecialchars("Los encabezados no son correctos. Se esperaba $headers[$i]; y se encontró: $documented_headers[$i].");
    exit_script($system_callback);
  }
}
